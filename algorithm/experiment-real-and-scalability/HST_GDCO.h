...
void _constructHST_greedy_(bool load, clock_t startClock, FUNC_PTR_TYPE func); // without any optimization
void _constructHST_greedy_algo0(bool load, clock_t startClock); // without any optimization
void _constructHST_greedy_algo3(bool load, clock_t startClock); // with reduction to NN queries, indexing, bounding

int _getCenter_maxDistor(partition_t& g, FUNC_PTR_TYPE func);
int _getCenter_maxDistor_complex(partition_t& g);

double _calc_maxDistor_bf(int center, partition_t& g);
