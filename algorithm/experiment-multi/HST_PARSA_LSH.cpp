...
static double eps_ann = 3.5;
static int LSH_THRESHOLD = 320;
static double QALSH_p = 2.0;
static double QALSH_zeta = 0.0;
static float* QALSH_data;
...
double _calc_maxDistor_rtree0_UB(int centerId, partition_t& g, double UB) {
	...
	QALSH<float> *lsh = NULL;
	MinK_List *list = new MinK_List(1);
	float queryPt[DIM_V];
	int nPts = 0, b = 0, e = 0;
	...
	for (int i=0,k=0,preLev=H+1,tid=0; i<npos&&maxDistor<UB; i=k,++tid) {
		...
		if (i>0 && nPts>0 && i<k) {
			...
			if (nPts > LSH_THRESHOLD) {
				for (int j=b,c=0; j<e; ++j) {
					int pid = g.ids[U[j]];
					for (int dim_i=0; dim_i<DIM_V; ++dim_i)
						QALSH_data[c++] = V[pid].x[dim_i];
				}
				lsh = new QALSH<float>(nPts, DIM_V, QALSH_p, QALSH_zeta, eps_ann+1.0, QALSH_data);
			}
		}
		...
		for (int j=i; j<k&&maxDistor<UB; ++j) {
			...
			for (int dim_i=0; dim_i<DIM_V; ++dim_i)
				queryPt[dim_i] = V[pid].x[dim_i];
			
			int kid = -1;
			double d;
			
			if (nPts > LSH_THRESHOLD) {
				lsh->knn(1, queryPt, list);
				kid = list->ith_id(0);
				qid = g.ids[U[b+kid]];
				d = list->ith_key(0);
			} else {
...
void initParameters(int flag, int m) {	
	static double pc = max(1.0, log(m)/log(2));
