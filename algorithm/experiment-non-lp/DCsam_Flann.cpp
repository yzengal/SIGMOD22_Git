...
void Algorithm3() {
	...
	double timeCost2 = 1.0 * (clock()-t1) / CLOCKS_PER_SEC;
	
	printf("DCsam-Flann: maxDistor=%.4lf, avgDistor=%.4lf, time=%.4lf, memory=%.4lf, time2=%.4lf\n", tmp.first, tmp.second, timeCost, usedMemory/1024.0, timeCost2);
	fflush(stdout);
}
